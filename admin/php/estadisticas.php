...
$stmt = $conn->query('SELECT COUNT(*) AS count FROM pedidos');
$pedidosCount = $stmt->fetch(PDO::FETCH_ASSOC)['count'];

// Contar registros en la tabla de usuarios
$stmt = $conn->query('SELECT COUNT(*) AS count FROM usuarios');
$usuariosCount = $stmt->fetch(PDO::FETCH_ASSOC)['count'];

// Contar registros en la tabla de productos
$stmt = $conn->query('SELECT COUNT(*) AS count FROM productos');
$productosCount = $stmt->fetch(PDO::FETCH_ASSOC)['count'];
...
<body>
  ...
  <main>
    <div class="tablas">
      <div class="botones">
        <div class="btn" id="Pedidos_Estadisticas">Pedidos Estadisticas</div>
        <div class="btn" id="Usuarios_Estadisticas">Usuarios Estadisticas</div>
        <div class="btn" id="Productos_Estadisticas">Productos Estadisticas</div>
      </div>
  ...
  <script>
   ...
   <script>
      ...
      fetch('../../php/logout.php../../../Proyecto_Fin_de_curso/login/login.php')
        .then(response => {
          if (response.ok) {
